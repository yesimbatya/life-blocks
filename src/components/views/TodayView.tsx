...
export function TodayView({ blocks, allocations, streak, allHabits, updateBlocks }: TodayViewProps) {
  ...
  const handleUpdateBlocks = useCallback((newBlocks: BlockAssignments) => updateBlocks(newBlocks), [updateBlocks])
    ...
    <div className="pb-24">
      ...
      <div className="px-5 mb-6 flex items-start justify-between">
        ...
        <button
          ...
          className="mt-2 bg-ios-card rounded-ios-lg p-3 shadow-ios ios-press"
      ...
      <div className="fixed bottom-16 left-0 right-0 bg-gradient-to-t from-ios-bg via-ios-bg to-transparent pt-10 pb-4 flex justify-center z-30 pointer-events-none">
        <button
          onClick={handleOpenTimeGrid}
          className="bg-ios-blue text-white rounded-xl px-6 py-4 shadow-lg flex items-center gap-3 ios-press pointer-events-auto"
        >
